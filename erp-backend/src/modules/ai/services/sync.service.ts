...
export class SyncService {
      ...
      await Promise.allSettled(
        batch.map(async (row) => {
          try {
            ...
            // Check hash — skip nếu content không đổi (nếu không force)
            if (!force) {
   ...
   */
  async fullSync(force: boolean = true): Promise<SyncJobResult[]> {
    const jobs: Array<{
      module: ERPModule;
      entityType: EntityType;
      sql: string;
    }> = [
      {
        module: "crm",
        entityType: "customer",
        sql: contentTemplates.crm.customer,
      },
      { module: "crm", entityType: "lead", sql: contentTemplates.crm.lead },
      {
        module: "purchase",
        entityType: "purchase_order",
        sql: contentTemplates.purchase.purchase_order,
      },
      {
        module: "purchase",
        entityType: "vendor",
        sql: contentTemplates.purchase.vendor,
      },
      {
        module: "sale",
        entityType: "sale_order",
        sql: contentTemplates.sale.sale_order,
      },
      {
        module: "inventory",
        entityType: "product",
        sql: contentTemplates.inventory.product,
      },
    ];
    ...
    for (const job of jobs) {
      try {
        ...
        console.error(
          `[Sync] Failed job ${job.module}/${job.entityType}:`,
          error,
        );
      }
    }
  ...
  async syncOne(
    ...
    const templates = contentTemplates as Record<
      string,
...
function getAllowedRoles(module: ERPModule): string[] {
  const base = [Role.ADMIN, Role.CEO, Role.BRANCH_MANAGER];

  if (module === "crm") {
    return [...base, Role.SALES, Role.SALESMANAGER];
  }
  if (module === "purchase") {
    return [
      ...base,
  ...
  if (module === "sale") {
    return [...base, Role.SALES, Role.SALESMANAGER, Role.ACCOUNT, Role.CHACC];
  }
  if (module === "inventory") {
    return [
      ...base,
