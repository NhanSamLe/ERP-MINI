...
export class ChatService {
  ...
  async chat(req: ChatRequest): Promise<ChatResponse> {
    ...
    // 1. Tìm context từ Qdrant — filter theo branch + role
    ...
    const contextText =
      sources.length > 0
        ? sources
            .map(
              (s) =>
                `[${s.entity_type.toUpperCase()} | score: ${s.score.toFixed(2)}]\n${s.content_text}`,
            )
            .join("\n\n")
...
${contextText}`;
    ...
    if (provider === "openai") {
      ...
      return {
        answer:
          response.choices[0]?.message?.content || "Không thể tạo phản hồi.",
        sources,
        model: activeModel,
      };
    } else {
  ...
  async *chatStream(req: ChatRequest): AsyncGenerator<any> {
    ...
    // Tìm context từ Qdrant — filter theo branch + role
    const queryVector = await embeddingService.embed(message);
    const searchOpts: {
      module?: ERPModule;
      top_k?: number;
