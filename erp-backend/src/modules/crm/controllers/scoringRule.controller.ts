...
export const getAll = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ message: "Lấy cấu hình chấm điểm (Scoring) thành công", data: rules });
  } catch (err: any) {
...
export const recalculateLead = async (req: Request, res: Response) => {
  try {
    ...
    const score = await scoringRuleService.calculateLeadScore(leadId);
    res.json({ message: "Đã tính lại điểm cho Lead", data: { score } });
  } catch (err: any) {
    res.status(400).json({ message: err.message });
  }
};
