...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const loginData = await authService.login({ username, password , rememberMe});
      dispatch(setToken(loginData.token));
      // 2. gọi API /auth/me → lấy thông tin user
      const user = await authService.getProfile();
      dispatch(setUser(user));
      // 3. nếu chưa setup xong → chuyển sang onboarding wizard
      if (loginData.is_setup_done === false) {
        navigate("/onboarding");
        return;
      }
      const rolePath = roleRoutes[user.role.code] || "/profile";
