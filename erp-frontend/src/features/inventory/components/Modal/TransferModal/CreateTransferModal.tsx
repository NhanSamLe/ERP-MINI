  ...
  const handleSubmit = () => {
    ...
    for (const p of lineItems) {
      ...
      if (!p.location_from_id) {
        toast.error(`Please select From Location for product: ${p.name}`);
        return;
      }
      if (!p.location_to_id) {
        toast.error(`Please select To Location for product: ${p.name}`);
        return;
      }
      if (!(p as any).lot_id) {
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="w-full sm:max-w-4xl max-h-[85vh] overflow-y-auto">
        <DialogHeader className="pb-4 border-b border-slate-100 flex flex-row items-center gap-3">
          ...
          <div>
            <DialogTitle className="text-lg font-bold text-slate-900">Create Stock Transfer</DialogTitle>
            <DialogDescription className="text-xs text-slate-400 mt-0.5">
              Move inventory products internally between warehouses
            </DialogDescription>
          </div>
        </DialogHeader>
