...
export default function BlogListPage() {
  ...
  const handleDelete = async (id: number, e: React.MouseEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!window.confirm("Bạn có chắc muốn xóa bài viết này? Thao tác này không thể hoàn tác.")) {
      return;
    }
    ...
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-gray-150 shadow-sm">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Quản lý và soạn bài quảng bá sản phẩm với sự hỗ trợ của trí tuệ nhân tạo (AI).
          </p>
        </div>
        <Button 
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-150 p-12 text-center flex flex-col items-center justify-center gap-4">
          ...
          <div>
            <h3 className="text-lg font-bold text-gray-800">Chưa có bài viết</h3>
            <p className="text-sm text-gray-500 mt-1 max-w-md">
              Bắt đầu soạn bài quảng bá sản phẩm đầu tiên bằng nút “Viết bài mới” ở trên.
            </p>
          </div>
          <Button 
            onClick={() => navigate("/blog/create")}
