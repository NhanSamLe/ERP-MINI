...
const GlAccountPage: React.FC = () => {
  ...
  const loadData = async (filter?: GlAccountFilter) => {
    try {
      ...
      console.error(e);
      toast.error(e.message || "Lỗi khi tải danh sách tài khoản");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (row: GlAccountDTO) => {
    if (!row.id) return;
    if (!window.confirm(`Bạn có chắc chắn muốn xóa tài khoản "${row.code} - ${row.name}" không?`)) return;
    try {
      setLoading(true);
      ...
      console.error(e);
      toast.error(e.message || "Xóa tài khoản thất bại");
    } finally {
      setLoading(false);
    }
  };
                  ...
                  <td colSpan={5} className="px-6 py-16">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 text-sm font-medium">
                        {search || typeFilter ? "Không tìm thấy kết quả phù hợp" : "Chưa có tài khoản nào"}
                      </p>
                      <p className="text-gray-400 text-xs mt-1">
                        {search || typeFilter
                          ? "Thử tìm kiếm với từ khóa khác"
                          : "Bấm 'Thêm tài khoản' để tạo mới"}
                      </p>
                    </div>
                  </td>
                </tr>
