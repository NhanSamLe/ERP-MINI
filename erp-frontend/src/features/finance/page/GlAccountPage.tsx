...
const GlAccountPage: React.FC = () => {
  ...
  const loadData = async (filter?: GlAccountFilter) => {
    try {
      ...
      console.error(e);
      toast.error(e.message || "Error loading account list");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async (row: GlAccountDTO) => {
    if (!row.id) return;
    if (!window.confirm(`Are you sure you want to delete account "${row.code} - ${row.name}"?`)) return;
    try {
      setLoading(true);
        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            ...
            <p className="text-gray-500 mt-2 text-sm">
              Total: <span className="font-semibold text-gray-700">{totalItems}</span> accounts
            </p>
          </div>
                  ...
                  <td colSpan={5} className="px-6 py-16">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-gray-500 text-sm font-medium">
                        {search || typeFilter ? "No matching results found" : "No accounts yet"}
                      </p>
                      <p className="text-gray-400 text-xs mt-1">
                        {search || typeFilter
                          ? "Try searching with other keywords"
                          : "Click 'Add Account' to create a new one"}
                      </p>
                    </div>
                  </td>
                </tr>
                ...
                pageItems.map((row) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-center">
                      ...
                      >
                        {row.normal_side === "debit" ? "Debit" : "Credit"}
                      </span>
                    </td>
                    <td className="px-4 py-3">
        ...
        {totalItems > 0 && (
          <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 border-t bg-gradient-to-r from-gray-50 to-white">
            <div className="text-sm text-gray-600">
              ...
              of{" "}
              <span className="font-semibold text-gray-900">{totalItems}</span> accounts
            </div>
